...
export default function track(params, force) {
    ...
    if (json.last === json.url && !R.defaultTo(false)(force))
        return;
    Cookies.set(process.env.CLIENT_REFERRAL_COOKIE, json.url, { expires: 1 });
    //Experiment
    ...
    //Duration
    let now = Date.now();
    let historical = R.defaultTo(now)(Cookies.get(process.env.CLIENT_TRACK_COOKIE));
    json.created = now;
    ...
    if (R.defaultTo(false)(sid)) {
        ...
        json.sid = uuidv1();
        json.first = "true";
        Cookies.set(process.env.CLIENT_SESS_COOKIE, json.sid, { expires: 1 });
    }
    //Timezone
